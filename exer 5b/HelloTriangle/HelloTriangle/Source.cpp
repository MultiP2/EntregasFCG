...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Eae Triangulo no Plural! - Jairo", nullptr, nullptr);
...
int setupShader()
{
	...
	float dist = min(min(gl_FragCoord.x, gl_FragCoord.y), min(1.0 - gl_FragCoord.x, 1.0 - gl_FragCoord.y));
	if (dist > 0.01) {
		discard;
	}
	// Checando erros de compilação (exibição via log no terminal)
